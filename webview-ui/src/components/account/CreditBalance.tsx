...
export const CreditBalance = ({ balance, fetchCreditBalance, creditUrl, lastFetchTime, isLoading }: CreditBalanceProps) => {
	return (
		<div
			...
			title={`Последнее обновление: ${new Date(lastFetchTime).toLocaleTimeString()}`}>
			<div className="text-sm text-(--vscode-descriptionForeground) mb-3 font-azeret-mono font-light">ТЕКУЩИЙ БАЛАНС</div>
