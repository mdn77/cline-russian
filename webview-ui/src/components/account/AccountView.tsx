...
const AccountView = ({ onDone, clineUser, organizations, activeOrganization }: AccountViewProps) => {
	...
	return (
		<div className="fixed inset-0 flex flex-col overflow-hidden">
			<ViewHeader environment={environment} onDone={onDone} showEnvironmentSuffix title="Аккаунт" />
			...
			<div className="flex flex-col h-full">
							...
							<div className="flex gap-2 items-center mt-1">
								<Tooltip>
									...
									<TooltipContent hidden={!isLockedByRemoteConfig}>
										Это нельзя изменить, пока для вашей организации включена удалённая конфигурация.
									</TooltipContent>
								</Tooltip>
								{activeOrganization && (
									<VSCodeTag className="text-xs p-2" title="Роль">
				...
				{isClineTester && environment !== "selfHosted" && (
					<div className="w-full gap-1 items-end">
						...
						<div className="text-sm font-semibold">Окружение Cline</div>
						<VSCodeDropdown
