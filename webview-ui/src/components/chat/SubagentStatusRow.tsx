	...
	return (
		<div className="relative">
			...
			{!isExpanded && showMoreVisible && (
				<button
					aria-label="Показать полный промпт сабагента"
...
export default function SubagentStatusRow({ message, isLast, lastModifiedMessage }: SubagentStatusRowProps) {
	...
	if (!data) {
		return <div className="text-foreground opacity-80">Обновление статуса сабагента недоступно.</div>
	}
	...
	const title = singular ? "Cline хочет использовать сабагента:" : "Cline хочет использовать сабагентов:"
		...
		<div className="mb-2">
			...
			<div className="space-y-2">
				{data.items.map((entry, index) => {
					...
					const statsText = `${formatCount(entry.toolCalls)} вызовов · ${formatCount(entry.contextTokens)} токенов · ${formatCost(entry.totalCost)}`
					...
					return (
						<div
							...
							{shouldShowStats && hasDetails && (
								<button
									aria-label={isExpanded ? "Скрыть вывод сабагента" : "Показать вывод сабагента"}
									...
									<span className="shrink-0">{isExpanded ? "Скрыть вывод" : "Показать вывод"}</span>
								</button>
							)}
