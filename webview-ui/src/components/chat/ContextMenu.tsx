...
const ContextMenu: React.FC<ContextMenuProps> = ({
	...
	const SIMPLE_OPTION_LABELS: Partial<Record<ContextMenuOptionType, string>> = {
		[ContextMenuOptionType.Problems]: "Проблемы",
		[ContextMenuOptionType.Terminal]: "Терминал",
		[ContextMenuOptionType.URL]: "Вставить URL для получения содержимого",
		[ContextMenuOptionType.NoResults]: "Ничего не найдено",
	}
	...
	const getOptionLabel = useCallback((option: ContextMenuQueryItem): string => {
		...
		switch (option.type) {
			...
			case ContextMenuOptionType.Folder:
				...
				return `Добавить ${option.type === ContextMenuOptionType.File ? "файл" : "папку"}`
			default:
				return option.label || option.value || ""
		}
	}, [])
	...
	const renderOptionContent = (option: ContextMenuQueryItem) => {
		...
		switch (option.type) {
			...
			case ContextMenuOptionType.Folder:
				...
				return <span>Добавить {option.type === ContextMenuOptionType.File ? "файл" : "папку"}</span>
			default:
				return null
		}
	}
	...
	return (
		<div
			...
			<div
				...
				aria-label="Упоминания контекста"
				ref={menuRef}
				role="listbox"
				style={{
