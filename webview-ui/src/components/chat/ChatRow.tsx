			...
			switch (type) {
				case "error":
					return [
						...
						<span className="text-error font-bold">Ошибка</span>,
					]
				case "mistake_limit_reached":
					return [
						<CircleXIcon className="text-error size-2" />,
						<span className="text-error font-bold">У Cline возникли трудности...</span>,
					]
				case "command":
					return [
						<TerminalIcon className="text-foreground size-2" />,
						<span className="font-bold text-foreground">Cline хочет выполнить эту команду:</span>,
					]
				case "use_mcp_server":
					...
					return [
						...
						<span className="ph-no-capture font-bold text-foreground break-words">
							Cline хочет {mcpServerUse.type === "use_mcp_tool" ? "использовать инструмент" : "получить доступ к ресурсу"} на MCP-сервере{" "}
				...
				case "completion_result":
					return [
						...
						<span className="text-success font-bold">Задача завершена</span>,
					]
				...
				case "followup":
					return [
						...
						<span className="font-bold text-foreground">Вопрос от Cline:</span>,
					]
				default:
					return [null, null]
			}
			...
			switch (tool.tool) {
				case "editedExistingFile":
					...
					const editToolTitle = isApplyingPatch
						? "Cline создает патчи для изменения этого файла:"
						: "Cline хочет изменить этот файл:"
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								<PencilIcon className="size-2" />
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "Этот файл находится за пределами вашей рабочей области")}
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "Этот файл находится за пределами вашей рабочей области")}
								<span style={{ fontWeight: "bold" }}>Cline хочет удалить этот файл:</span>
							</div>
							<CodeAccordian
								// isLoading={message.partial}
								code={tool.content}
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "Этот файл находится за пределами вашей рабочей области")}
								<span className="font-bold">Cline хочет создать новый файл:</span>
							</div>
							{backgroundEditEnabled && tool.path && tool.content ? (
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "Этот файл находится за пределами вашей рабочей области")}
								<span className="font-bold">Cline хочет прочитать этот файл:</span>
							</div>
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "Это находится за пределами вашей рабочей области")}
								<span style={{ fontWeight: "bold" }}>
									{message.type === "ask"
										? "Cline хочет просмотреть файлы верхнего уровня в этой директории:"
										: "Cline просмотрел файлы верхнего уровня в этой директории:"}
								</span>
							</div>
							<CodeAccordian
								code={tool.content!}
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span style={{ fontWeight: "bold" }}>
									{message.type === "ask"
										? "Cline хочет рекурсивно просмотреть все файлы в этой директории:"
										: "Cline рекурсивно просмотрел все файлы в этой директории:"}
								</span>
							</div>
							<CodeAccordian
								code={tool.content!}
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span style={{ fontWeight: "bold" }}>
									{message.type === "ask"
										? "Cline хочет просмотреть имена определений исходного кода в этой директории:"
										: "Cline просмотрел имена определений исходного кода в этой директории:"}
								</span>
							</div>
							<CodeAccordian
								code={tool.content!}
						...
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span className="font-bold">
									Cline хочет выполнить поиск в этой директории по выражению <code className="break-all">{tool.regex}</code>:
								</span>
							</div>
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span className="font-bold">Cline сжимает контекст беседы:</span>
							</div>
									...
									{isExpanded ? (
										<div>
											<div className="flex items-center mb-2">
												<span className="font-bold mr-1">Сводка:</span>
												<div className="grow" />
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "Этот URL является внешним")}
								<span className="font-bold">
									{message.type === "ask"
										? "Cline хочет загрузить содержимое по этому URL:"
										: "Cline загрузил содержимое по этому URL:"}
								</span>
							</div>
							<div
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "Этот поиск является внешним")}
								<span className="font-bold">
									{message.type === "ask"
										? "Cline хочет выполнить поиск в сети по запросу:"
										: "Cline выполнил поиск в сети по запросу:"}
								</span>
							</div>
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span className="font-bold">Cline загрузил навык:</span>
							</div>
						...
						return (
							<div className="flex flex-col bg-warning/20 p-2 rounded-xs border border-error">
								<div className="flex items-center mb-1">
									...
									<span className="font-medium text-foreground">Интеграция с оболочкой недоступна</span>
								</div>
								<div className="text-foreground opacity-80">
									Cline может испытывать проблемы с отображением вывода команды. Пожалуйста, обновите VSCode (
									<code>CMD/CTRL + Shift + P</code> → "Update") и убедитесь, что используется поддерживаемая оболочка:
									zsh, bash, fish или PowerShell (<code>CMD/CTRL + Shift + P</code> → "Terminal: Select Default
									Profile").
									<a
										className="px-1"
									...
									<div className="flex flex-col bg-quote p-0 rounded-[3px] text-[12px] p-3">
										<div className="flex items-center mb-1">
											...
											<span className="font-medium text-foreground">
												{isFailed ? "Автоповтор не удался" : "Автоповтор..."}
											</span>
										</div>
										<div className="text-foreground opacity-80">
											{isFailed ? (
												<span>
													Автоповтор не удался после <strong>{maxAttempts}</strong> попыток. Требуется
													ручное вмешательство.
												</span>
											) : (
												<span>
													Попытка <strong>{attempt}</strong> из <strong>{maxAttempts}</strong> —
													Повтор через {delaySeconds} секунд...
												</span>
											)}
										</div>
									</div>
						...
						return (
							<div className="p-2 bg-link/10 border border-link/30 rounded-xs">
								<div className="flex items-center mb-1">
									...
									<span className="font-medium text-foreground">Проблемы интеграции оболочки</span>
								</div>
								<div className="text-foreground opacity-90 mb-2">
									Если вы испытываете повторяющиеся проблемы с интеграцией оболочки, рекомендуем переключиться на
									режим фонового терминала для повышения надёжности.
								</div>
								<button
									className={cn(
									...
									{isBackgroundModeEnabled
										? "Фоновый терминал включён"
										: "Включить фоновый терминал (рекомендуется)"}
								</button>
							</div>
						)
					case "task_progress":
					...
					case "new_task":
						return (
							<div>
								<div className={HEADER_CLASSNAMES}>
									<FilePlus2Icon className="size-2" />
									<span className="text-foreground font-bold">Cline хочет начать новую задачу:</span>
								</div>
					...
					case "condense":
						return (
							<div>
								<div className={HEADER_CLASSNAMES}>
									<FilePlus2Icon className="size-2" />
									<span className="text-foreground font-bold">Cline хочет сократить ваш диалог:</span>
								</div>
								<NewTaskPreview context={message.text || ""} />
							</div>
						)
					case "report_bug":
						return (
							<div>
								<div className={HEADER_CLASSNAMES}>
									<FilePlus2Icon className="size-2" />
									<span className="text-foreground font-bold">Cline хочет создать issue на GitHub:</span>
								</div>
