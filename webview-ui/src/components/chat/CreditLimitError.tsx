...
const CreditLimitError: React.FC<CreditLimitErrorProps> = ({
	message = "У вас закончились кредиты.",
	buyCreditsUrl,
	currentBalance,
	totalPromotions,
	totalSpent,
				...
				<div className="mb-3">
					{currentBalance ? (
						<div className="text-foreground">
							Текущий баланс: <span className="font-bold">{currentBalance.toFixed(2)}</span>
						</div>
					) : null}
					{totalSpent ? <div className="text-foreground">Всего потрачено: {totalSpent.toFixed(2)}</div> : null}
					{totalPromotions ? (
						<div className="text-foreground">Всего промоакций: {totalPromotions.toFixed(2)}</div>
					) : null}
				</div>
			</div>
