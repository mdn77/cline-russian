		...
		<BrowserSessionRowContainer style={{ marginBottom: -10 }}>
			<div style={browserSessionRowContainerInnerStyle}>
				...
				<span style={approveTextStyle}>
					{isAutoApproved ? "Cline использует браузер:" : "Cline хочет использовать браузер:"}
				</span>
			</div>
			<div
				style={{
				...
				<div style={consoleLogsContainerStyle}>
					<div
						...
						<span style={consoleLogsTextStyle}>Логи консоли</span>
					</div>
					{consoleLogsExpanded && (
						<CodeBlock source={`${"```"}shell\n${displayState.consoleLogs || "(Нет новых логов)"}\n${"```"}`} />
					)}
				</div>
			</div>
			...
			{pages.length > 1 && (
				<div style={paginationContainerStyle}>
					<div>
						Шаг {currentPageIndex + 1} из {pages.length}
					</div>
			...
			return (
				<>
					<div style={headerStyle}>
						<span style={browserSessionStartedTextStyle}>Сессия браузера запущена</span>
					</div>
...
const BrowserActionBox = ({ action, coordinate, text }: { action: BrowserAction; coordinate?: string; text?: string }) => {
	const getBrowserActionText = (action: BrowserAction, coordinate?: string, text?: string) => {
		switch (action) {
			case "launch":
				return `Запустить браузер: ${text}`
			case "click":
				return `Клик (${coordinate?.replace(",", ", ")})`
			case "type":
				return `Ввести "${text}"`
			case "scroll_down":
				return "Прокрутить вниз"
			case "scroll_up":
				return "Прокрутить вверх"
			case "close":
			...
			<div style={browserActionBoxContainerInnerStyle}>
				<div style={browseActionRowContainerStyle}>
					<span style={browseActionRowStyle}>
						<span style={browseActionTextStyle}>Действие в браузере: </span>
