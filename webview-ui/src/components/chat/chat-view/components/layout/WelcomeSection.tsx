...
/**
 * Приветственная секция, когда нет активной задачи
 * Включает информационные баннеры, объявления, заголовок и историю
 */
export const WelcomeSection: React.FC<WelcomeSectionProps> = ({
	...
	// Отслеживаем, показали ли модалку «Что нового» в этой сессии
	const [hasShownWhatsNewModal, setHasShownWhatsNewModal] = useState(false)
	const [showWhatsNewModal, setShowWhatsNewModal] = useState(false)

	// Модалка быстрого создания worktree
	...
	// Проверяем, есть ли git-репозиторий, и получаем информацию о текущем worktree
	useEffect(() => {
	...
	// Открываем модалку, когда есть приветственные баннеры
	useEffect(() => {
	...
	// Обработка клика по элементу worktree на главной странице с телеметрией
			...
			<div className="overflow-y-auto flex flex-col pb-2.5">
				...
				{!showWhatsNewModal && (
					<>
						...
						{/* Кнопка быстрого запуска worktree */}
							...
							<div className="flex flex-col items-center gap-3 mt-2 mb-4 px-5">
								...
								<Tooltip>
									...
									<TooltipContent side="top">
										Создать новый git worktree и открыть его в отдельном окне. Удобно для параллельных
										задач Cline.
									</TooltipContent>
								</Tooltip>
								*/}
								{currentWorktree && (
									<Tooltip>
											...
											<button
												...
												<div className="flex items-center gap-1.5 text-xs">
													...
													<span className="break-all text-center">
														<span className="font-semibold">Текущий:</span>{" "}
										...
										<TooltipContent side="bottom">
											Просмотр и управление git worktrees. Удобно для параллельных задач Cline.
										</TooltipContent>
									</Tooltip>
								)}
							</div>
