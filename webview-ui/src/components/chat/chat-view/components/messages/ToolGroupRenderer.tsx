...
const getActivityText = (tool: ClineSayTool): string | null => {
	...
	const formatSearchRegex = (regex: string, path: string, filePattern?: string): string => {
		...
		return filePattern && filePattern !== "*"
			? `"${terms}" в ${cleanedPath}/ (${filePattern})`
			: `"${terms}" в ${cleanedPath}/`
	}
	...
	switch (tool.tool) {
		case "readFile": {
			...
			return `Чтение ${cleanedPath}${lineHint}...`
		}
		case "listFilesTopLevel":
		case "listFilesRecursive":
			return tool.path ? `Обзор ${cleanedPath}/...` : null
		case "searchFiles":
			return tool.regex && tool.path ? `Поиск ${formatSearchRegex(tool.regex, tool.path, tool.filePattern)}...` : null
		case "listCodeDefinitionNames":
			return tool.path ? `Анализ ${cleanedPath}/...` : null
		default:
			return null
	}
}
...
function getToolDisplayInfo(tool: ClineSayTool) {
	...
	switch (tool.tool) {
		case "readFile": {
			const lineNote =
				tool.readLineStart != null && tool.readLineEnd != null ? `строки ${tool.readLineStart}-${tool.readLineEnd}` : null
			return {
				icon,
				path: filePath,
				label: "read",
...
function formatSearchDisplay(regex: string, path: string, filePattern?: string): string {
	...
	const termDisplay = terms.length > 3 ? `${terms.length} шаблонов` : `"${terms.join(" | ")}"`
	let result = `${termDisplay} в ${cleanPathPrefix(path)}/`
 ...
 */
function pluralize(count: number, singular: string, genitiveSingular: string, genitivePlural: string): string {
	if (count % 10 === 1 && count % 100 !== 11) return `${count} ${singular}`
	if (count % 10 >= 2 && count % 10 <= 4 && (count % 100 < 10 || count % 100 >= 20)) return `${count} ${genitiveSingular}`
...
export function getToolGroupSummaryFromParsedTools(tools: ClineSayTool[]): string {
	...
	if (counts.read > 0) {
		parts.push(pluralize(counts.read, "файл", "файла", "файлов"))
	}
	if (counts.list > 0) {
		parts.push(pluralize(counts.list, "папка", "папки", "папок"))
	}
	if (counts.def > 0) {
		parts.push(pluralize(counts.def, "определение", "определения", "определений"))
	}
	if (counts.search > 0) {
		parts.push(pluralize(counts.search, "поиск", "поиска", "поисков"))
	}

	return parts.length === 0 ? "Контекст" : "Cline: " + parts.join(", ")
}
