...
const AutoApproveBar = ({ style }: AutoApproveBarProps) => {
	...
	const getEnabledActionsText = () => {
		...
		if (actionsToShow.length === 0) {
			return <span className={baseClasses}>Нет</span>
		}
		...
		return (
			<div
				...
				<div className="pt-4 pb-3.5 px-3.5">
					<div className="text-sm mb-1">Автоподтверждение: YOLO</div>
	...
	return (
		<div
			...
			<div
				aria-label={isModalVisible ? "Закрыть настройки автоподтверждения" : "Открыть настройки автоподтверждения"}
				...
				<div className="flex flex-nowrap items-center gap-1 min-w-0 flex-1">
					<span className="whitespace-nowrap">Автоподтверждение:</span>
