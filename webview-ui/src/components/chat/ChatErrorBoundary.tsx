		...
		if (this.state.hasError) {
			return (
				<div
					...
					<h3 style={{ margin: "0 0 8px 0" }}>{errorTitle || "Что-то пошло не так при отображении этого содержимого"}</h3>
					<p style={{ margin: "0" }}>{errorBody || `Ошибка: ${this.state.error?.message || "Неизвестная ошибка"}`}</p>
				</div>
			)
		}
