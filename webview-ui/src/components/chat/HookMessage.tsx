...
const HookMessage = memo(({ message, CommandOutput }: HookMessageProps) => {
	...
	const { metadata, output } = useMemo(() => {
		...
		try {
			...
			hookMetadata = { hookName: "Неизвестно", status: "unknown" }
		}
	...
	return (
		<>
			<div style={headerStyle}>
				...
				<span style={{ color: normalColor, fontWeight: "bold" }}>Хук:</span>
...
Прервать
				...
				{isFailed && metadata.error && metadata.error.type === "timeout" && (
					<div
						...
						Выполнение заняло более 30 секунд. Проверьте наличие бесконечных циклов или добавьте таймауты к сетевым запросам.
					</div>
				)}
				...
				{isFailed && metadata.error && metadata.error.type === "validation" && (
					<div
						...
						Хук вернул невалидный JSON. Подробности ошибки ниже.
					</div>
				)}
