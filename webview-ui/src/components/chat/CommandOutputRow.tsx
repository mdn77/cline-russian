			...
			<>
									...
									<Button
										onClick={(e) => {
											...
											if (isBackgroundExec) {
												...
												alert(
													"Команда выполняется в терминале VSCode. Вы можете остановить её с помощью Ctrl+C в терминале или включить фоновый режим в настройках для отменяемых команд.",
												)
											}
										}}
										size="sm"
										variant="secondary">
										{isBackgroundExec ? "отмена" : "стоп"}
									</Button>
								)}
							</div>
						</div>
				...
				{requestsApproval && (
					<div className="flex items-center gap-2.5 p-2 text-[12px] text-editor-warning-foreground">
						...
						<span>Модель определила, что эта команда требует явного подтверждения.</span>
					</div>
				)}
			</>
		)
