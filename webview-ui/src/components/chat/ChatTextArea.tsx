	...
	(
		...
		const providerOptions = useMemo(() => {
			...
			if (remoteProviders.length > 0) {
				providers = providers.filter((option) => remoteProviders.includes(option.value as any))
			}
			return providers
		}, [remoteConfigSettings])
				...
				<div
					...
					{showDimensionError && (
						<div className="absolute inset-2.5 bg-[rgba(var(--vscode-errorForeground-rgb),0.1)] border-2 border-error rounded-xs flex items-center justify-center z-10 pointer-events-none">
							<span className="text-error font-bold text-xs text-center">Размер изображения превышает 7500px</span>
						</div>
					)}
					{showUnsupportedFileError && (
						<div className="absolute inset-2.5 bg-[rgba(var(--vscode-errorForeground-rgb),0.1)] border-2 border-error rounded-xs flex items-center justify-center z-10 pointer-events-none">
							<span className="text-error font-bold text-xs">Файлы, кроме изображений, временно отключены</span>
						</div>
					)}
					{showSlashCommandsMenu && (
					...
					{!inputValue && selectedImages.length === 0 && selectedFiles.length === 0 && (
						<div className="text-xs absolute bottom-5 left-6.5 right-16 text-(--vscode-input-placeholderForeground)/50 whitespace-nowrap overflow-hidden text-ellipsis pointer-events-none z-1">
							Введите @ для контекста, / для команд и workflow, удерживайте shift для перетаскивания
						</div>
					)}
				...
				<div className="flex justify-between items-center -mt-[2px] px-3 pb-2">
					...
					<div className="relative flex-1 min-w-0 h-5">
						...
						<ButtonGroup className="absolute top-0 left-0 right-0 ease-in-out w-full h-5 z-10 flex items-center">
							<Tooltip>
								<TooltipContent>Добавить контекст</TooltipContent>
								<TooltipTrigger>
									<VSCodeButton
										appearance="icon"
							...
							<Tooltip>
								<TooltipContent>Добавить файлы и изображения</TooltipContent>
								<TooltipTrigger>
									<VSCodeButton
										appearance="icon"
										aria-label="Добавить файлы и изображения"
										className="p-0 m-0 flex items-center"
							...
							<ModelContainer>
								<ModelButtonWrapper style={{ position: "relative" }}>
									<ModelDisplayButton
										disabled={false}
										onClick={handleModelButtonClick}
										role="button"
										tabIndex={0}
									    title="Выбрать провайдера API">
										<ModelButtonContent className="text-xs">
											{modelDisplayName} <span className="codicon codicon-chevron-up" style={{ fontSize: "8px", marginLeft: "2px" }} />
										</ModelButtonContent>
									</ModelDisplayButton>
									{showProviderDropdown && (
										<div
										...
										>
											...
											>
												...
												<div style={{ height: "1px", backgroundColor: "var(--vscode-editorGroup-border)", margin: "4px 0" }} />
												<div
													style={{
														padding: "6px 10px",
														cursor: "pointer",
												...
												>
													<span className="codicon codicon-gear" style={{ fontSize: "11px" }} />
					...
					<Tooltip>
						<TooltipContent
							...
							side="top">
							{`В режиме ${shownTooltipMode === "act" ? "«Act»" : "«Plan»"} Cline ${shownTooltipMode === "act" ? "сразу выполнит задачу" : "соберёт информацию для составления плана"}`}
							<p className="text-description/80 text-xs mb-0">
								Переключение: <kbd className="text-muted-foreground mx-1">{togglePlanActKeys}</kbd>
							</p>
						</TooltipContent>
						<TooltipTrigger>
							<SwitchContainer data-testid="mode-switch" disabled={false} onClick={onModeToggle}>
								<Slider isAct={mode === "act"} isPlan={mode === "plan"} />
								{[{ id: "plan", label: "План" }, { id: "act", label: "Действие" }].map((m) => (
									<div
										aria-checked={mode === m.id}
										className={cn(
											"pt-0.5 pb-px px-2 z-10 text-xs w-1/2 text-center bg-transparent",
											mode === m.id ? "text-white" : "text-input-foreground",
										)}
										onMouseLeave={() => setShownTooltipMode(null)}
										onMouseOver={() => setShownTooltipMode(m.id as Mode)}
										role="switch">
										{m.label}
									</div>
								))}
