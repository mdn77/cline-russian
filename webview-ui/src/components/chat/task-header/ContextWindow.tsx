...
const ConfirmationDialog = memo<{
	...
	<div className="text-sm my-2 flex items-center gap-0 justify-between">
		<span className="font-semibold text-sm">Свернуть текущую задачу?</span>
		<span className="flex gap-1">
			<VSCodeButton
				appearance="secondary"
				className="text-sm"
				onClick={onCancel}
				title="Нет, оставить как есть"
				type="button">
				Отмена
			</VSCodeButton>
			<VSCodeButton
		...
		<div className="flex flex-col my-1.5" onMouseLeave={debounceCloseHover}>
			<div className="flex gap-1 flex-row @max-xs:flex-col @max-xs:items-start items-center text-sm">
				<div className="flex items-center gap-1.5 flex-1 whitespace-nowrap">
					<span className="cursor-pointer text-sm" title="Текущее количество токенов в этом запросе">
						...
						<HoverCard>
							...
							<HoverCardTrigger asChild>
								...
								<div
									...
									ref={progressBarRef}>
				<Progress
					aria-label="Использование контекстного окна"
										color="success"
					...
					<span className="cursor-pointer text-sm" title="Максимальный размер контекстного окна для этой модели">
