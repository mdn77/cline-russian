...
const COMPLETED_MESSAGE = "Все задачи выполнены!"
const TODO_LIST_LABEL = "Список задач"
const NEW_STEPS_MESSAGE = "Новые шаги будут сгенерированы, если вы продолжите задачу"
const CLICK_TO_EDIT_TITLE = "Нажмите, чтобы редактировать список задач в файле"
...
export const FocusChain: React.FC<FocusChainProps> = memo(
	({ currentTaskItemId, lastProgressMessageText, showPlaceholderWhenEmpty }) => {
		...
		if (!todoInfo) {
			...
			return (
				<div
					...
					<span className="text-sm text-foreground/80 truncate">Задачи</span>
				</div>
			)
		}
		...
		return (
			<div
				aria-label={isExpanded ? "Свернуть список задач" : "Развернуть список задач"}
