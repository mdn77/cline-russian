...
const TOKEN_DETAILS_CONFIG: Omit<TokenDetail, "value">[] = [
	{ title: "Токены запроса", icon: "codicon-arrow-up" },
	{ title: "Токены ответа", icon: "codicon-arrow-down" },
	{ title: "Запись кэша", icon: "codicon-arrow-left" },
	{ title: "Чтение кэша", icon: "codicon-arrow-right" },
]
...
const TokenUsageDetails = memo<TokenUsageInfoProps>(({ tokensIn, tokensOut, cacheWrites, cacheReads }) => {
	...
	if (!tokensIn) {
		return <div>Нет данных об использовании токенов</div>
	}
		...
		<div className="context-window-tooltip-content flex flex-col gap-2 bg-menu rounded shadow-sm z-100 w-60 p-1">
			{autoCompactThreshold > 0 && (
				<AccordionItem
					...
					title="Порог авто-сжатия"
					value={<span className="text-muted-foreground">{`${(autoCompactThreshold * 100).toFixed(0)}%`}</span>}>
					<div className="space-y-1">
						<p className="text-xs leading-relaxed text-white">
							Нажмите на полосу контекстного окна, чтобы установить новый порог.
						</p>
						<p className="text-xs leading-relaxed mt-0 mb-0">
							Когда использование контекстного окна превысит этот порог, задача будет автоматически сжата.
						</p>
					</div>
				</AccordionItem>
			)}
