...
const HISTORY_FILTERS = {
	...
	mostTokens: "Больше токенов",
	mostRelevant: "Релевантные",
	workspaceOnly: "Только workspace",
...
const HistoryView = ({ onDone }: HistoryViewProps) => {
	...
	const { groupedTasks, groupCounts, groupLabels } = useMemo(() => {
		...
		if (todayTasks.length > 0) {
			groups.push({ tasks: todayTasks, label: "Сегодня" })
		}
		if (olderTasks.length > 0) {
			groups.push({ tasks: olderTasks, label: "Ранее" })
		}
	...
	return (
		<div className="fixed overflow-hidden inset-0 flex flex-col w-full">
			{/* HEADER */}
			<ViewHeader environment={environment} onDone={onDone} title="История" />
			...
			<div className="flex flex-col gap-3 px-3">
				...
				<div className="flex justify-between items-center">
					...
					<VSCodeTextField
						...
						placeholder="Нечёткий поиск по истории..."
						value={searchQuery}>
			...
			<div className="p-2.5 border-t border-t-border-panel">
				...
				{selectedItems.length > 0 ? (
					<Button
						...
						variant="danger">
						Удалить выбранное{selectedItems.length > 1 ? ` (${selectedItems.length})` : ""}
						{selectedItemsSize > 0 ? ` (${formatSize(selectedItemsSize)})` : ""}
					</Button>
				) : (
					<Button
						aria-label="Удалить всю историю"
						className="w-full"
						...
						variant="danger">
						Удалить всю историю{totalTasksSize !== null ? ` (${formatSize(totalTasksSize)})` : ""}
					</Button>
				)}
			</div>
		</div>
