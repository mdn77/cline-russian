	...
	implements vscode.WebviewViewProvider
{
	...
	public async resolveWebviewView(
		...
		Logger.log("[VscodeWebviewProvider] Calling getHtmlContent...");
		const html = this.getHtmlContent();
		Logger.log("[VscodeWebviewProvider] HTML length: " + html.length);
		Logger.log(
			"[VscodeWebviewProvider] HTML starts with: " + html.substring(0, 200),
		);
		Logger.log(
			"[VscodeWebviewProvider] HTML has test div: " +
				html.includes('id="test"'),
		);
		Logger.log(
			"[VscodeWebviewProvider] HTML title: " +
				html.match(/<title>(.*?)<\/title>/)?.[1],
		);
		webviewView.webview.html = html;
		Logger.log("[VscodeWebviewProvider] HTML set successfully");
