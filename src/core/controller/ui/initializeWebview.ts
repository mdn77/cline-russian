...
export async function initializeWebview(
	...
	try {
		...
		refreshOpenRouterModels(controller)
			...
			.catch((e) => Logger.error("refreshOpenRouterModels failed:", e));
		...
		refreshClineModels(controller)
			...
			.catch((e) => Logger.error("refreshClineModels failed:", e));
		...
		refreshGroqModels(controller)
			...
			.catch((e) => Logger.error("refreshGroqModels failed:", e));
		...
		refreshBasetenModels(controller)
			...
			.catch((e) => Logger.error("refreshBasetenModels failed:", e));
		...
		refreshHicapModels(controller, EmptyRequest.create())
			...
			.catch((e) => Logger.error("refreshHicapModels failed:", e));
		...
		if (liteLlmBaseUrl && liteLlmApiKey) {
			try {
				...
				Logger.error("refreshLiteLlmModels failed:", e);
			}
		}
		...
		// Silently refresh MCP marketplace catalog (wrapped - network call)
		try {
			controller.refreshMcpMarketplace(true /* sendCatalogEvent */);
		} catch (e) {
			Logger.error("refreshMcpMarketplace failed:", e);
		}
		...
		controller
			...
			.catch((e) => Logger.error("telemetry init failed:", e));
