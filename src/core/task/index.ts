...
import { LanguageDisplay } from "@shared/Languages"
...
export class Task {
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		const preferredLanguageDisplay = preferredLanguageRaw as LanguageDisplay
		// Extract English name from "Russian - Русский" format -> "Russian" or "English" -> "English"
		const preferredLanguage = preferredLanguageDisplay
			? preferredLanguageDisplay.split(" - ")[0].trim()
			: undefined
