...
export async function activate(context: vscode.ExtensionContext) {
	...
	let webview: VscodeWebviewProvider
	Logger.log("[DIAG] Starting initialize...")
	try {
		webview = (await initialize(storageContext)) as VscodeWebviewProvider
	} catch (e) {
		Logger.error("[Cline] initialize() failed catastrophically (continuing with stub):", e)
		// Re-throw — the host will still try to render the webview, but we want the
		// error to be visible in the Output panel.
		throw e
	}
	Logger.log("[DIAG] initialize() returned OK, type=" + typeof webview + " instance=" + (webview instanceof VscodeWebviewProvider))

	// 5. Register services and commands specific to VS Code
	// Initialize test mode and add disposables to context
	Logger.log("[DIAG] Starting initializeTestMode...")
	const testModeWatchers = await initializeTestMode(webview)
	Logger.log("[DIAG] initializeTestMode returned OK, watchers=" + testModeWatchers.length)
	context.subscriptions.push(...testModeWatchers)

	// Initialize hook discovery cache for performance optimization
	Logger.log("[DIAG] Starting HookDiscoveryCache.initialize...")
	...
	Logger.log("[DIAG] HookDiscoveryCache.initialize returned OK")

	Logger.log("[DIAG] Registering WebviewViewProvider...")
	context.subscriptions.push(
		vscode.window.registerWebviewViewProvider(VscodeWebviewProvider.SIDEBAR_ID, webview, {
			webviewOptions: { retainContextWhenHidden: true },
		}),
	)
	Logger.log("[DIAG] WebviewViewProvider registered OK")
